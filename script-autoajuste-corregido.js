...
function filtrar() {
  ...
  const filtrados = datosOriginales.filter(row => {
    const valorContraparte = (row['Contraparte'] || '').trim().toLowerCase();
    const valorEjercicio = (row['Ejercicio'] || '').trim().toLowerCase();
    const valorConcepto = (row['Concepto'] || '').trim().toLowerCase();
    return (!contraparte || valorContraparte === contraparte.trim().toLowerCase()) &&
           (!ejercicio || valorEjercicio === ejercicio.trim().toLowerCase()) &&
           (!concepto || valorConcepto === concepto.trim().toLowerCase());
  });
